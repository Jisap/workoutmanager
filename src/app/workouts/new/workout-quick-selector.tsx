...
export function WorkoutQuickSelector({
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/90 dark:border-gray-700 p-4 sm:p-5 shadow-xs space-y-4">
      ...
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="w-full flex items-center justify-between gap-3 px-3.5 py-3 bg-gray-50/90 dark:bg-gray-800/50 hover:bg-gray-100/90 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-left transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/20"
        >
          <div className="flex items-center gap-3 min-w-0">
            <div
              className={`p-2.5 rounded-xl shrink-0 ${
                activeTab === 'recent' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300'
              }`}
            >
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                {activeTab === 'recent' && selectedRecent?.typeName && (
                  <span className="px-2 py-0.5 text-[10px] font-semibold bg-gray-200/80 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-md">
                    {selectedRecent.typeName}
                  </span>
                )}
                {activeTab === 'templates' && selectedTemplate?.typeName && (
                  <span className="px-2 py-0.5 text-[10px] font-semibold bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 rounded-md">
          ...
          <ChevronDown
            className={`w-4 h-4 text-gray-400 dark:text-gray-500 shrink-0 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
          />
        </button>

        {/* Menú Desplegable con Buscador interno */}
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
            {/* Buscador dentro del Combobox */}
            <div className="p-2.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-800/50">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-1.5 text-xs bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 dark:text-gray-100"
                  autoFocus
                />
              </div>
            </div>
            ...
            <div className="max-h-64 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800">
              {currentList.length === 0 ? (
                <div className="p-5 text-center text-xs text-gray-500 dark:text-gray-400">
                      ...
                      className={`w-full text-left px-3.5 py-3 flex items-center justify-between text-xs transition-colors ${
                        isSelected
                          ? isRecentItem
                            ...
                            : 'bg-purple-50 dark:bg-purple-900/20 text-purple-900 dark:text-purple-300 font-semibold'
                    ...
                    >
                      <div className="min-w-0 flex-1 pr-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="truncate font-bold text-gray-900 dark:text-gray-100">{item.name}</span>
                          {item.typeName && (
                            <span className="px-1.5 py-0.2 rounded text-[10px] bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 font-medium">
                              {item.typeName}
                            </span>
                          )}
                        </div>
                      ...
                      <span className="text-[10px] text-gray-400 dark:text-gray-500 shrink-0 text-right max-w-[140px] truncate">
      ...
      {((activeTab === 'recent' && selectedRecent) || (activeTab === 'templates' && selectedTemplate)) && (
        <div className="bg-gray-50/90 dark:bg-gray-800/50 rounded-2xl p-4 sm:p-5 border border-gray-200/90 dark:border-gray-700 space-y-4 animate-in fade-in duration-200">
...
Completado el{' '}
          ...
          {activeTab === 'recent' && selectedRecent?.notes && (
            <div className="p-3 bg-amber-50/70 dark:bg-amber-900/20 border border-amber-200/80 dark:border-amber-800 rounded-xl text-xs text-amber-900 dark:text-amber-200">
              ...
              <p className="italic text-amber-800 dark:text-amber-300">{selectedRecent.notes}</p>
            </div>
          )}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider flex items-center gap-1.5">
                <Dumbbell className="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" /> Desglose de Ejercicios
              </span>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-xs text-gray-900 dark:text-gray-100 flex items-center gap-1.5">
                          <span className="text-[10px] w-4 h-4 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-bold flex items-center justify-center">
                          {idx + 1}
                        </span>
                        {ex.name}
                      </span>
                      {ex.maxWeight > 0 && (
                        <span className="text-[11px] font-semibold text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-900/20 px-2 py-0.5 rounded-md">
                          Máx: {ex.maxWeight} kg
                        </span>
                      )}
                    </div>
